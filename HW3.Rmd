...
<mark>Comparing the cluster dendrogram, we see that: 1. `complete` and `average` linkage produces two groups of genes, but the gene composition with in groups are different; for the `single` linkage, we failed to distinguish any obvious "groups".  Thus we conclude that the result does depend on the type of linkage selected.

### (c) 

<mark>To give a best representation of which genes differ the most, we can do the following: 1. Calculate the absolute mean difference between the healthy and diseased individuals, and 2. Do a 2-sample t-test between the two groups.
...
for (i in 1:nrow(data)) {
  healthy_data <- data[i, 1:20]  
  diseased_data <- data[i, 21:40] 
  p_val[i] <- t.test(healthy_data, diseased_data)$p.value
}

top_genes <- order(p_val)[1:5]

gene_names <- rownames(data)[top_genes]
gene_names
p_val[top_genes]
```

<mark>From the result of `abs_mean_diff` and `top_genes`, we can see that `gene600`, `gene502` are on the top of both lists, so they differ the most overall across the two groups, `healthy` and `diseased` individuals.
