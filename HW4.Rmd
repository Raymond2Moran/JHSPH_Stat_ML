...
plot(shrinkage_val, training_mse, type = 'b', pch = 19, col = "lightblue",
     xlab = "Shrinkage Parameter (λ)", ylab = "Training Set MSE",
     main = "Training Set MSE vs. Shrinkage Parameter λ")
```
...
plot(shrinkage_val, test_mse, type = "b", pch = 19, col = "lightblue",
     xlab = "Shrinkage Parameter (λ)", ylab = "Test Set MSE",
     main = "Test Set MSE vs. Shrinkage Parameter λ")
...
<mark>I will fit a **simple linear regression model** and a **Ridge regression model**, and then calculate & compare test MSE.
...
<mark>Based on the results of test MSE, we can observe that for the optimal value of $\lambda \ (0.01)$, the boosting method on the test set performed better than both simple linear regression & Ridge regression model.
...
<mark>From both the importance table & the figure, we can conclude that variables `CAtBat`, `PutOuts`, `Walks` are the three most important ones.
...
<mark>So the MSE of Bagging is approximately $0.2316$, this is lower than the value of Boosting, Linear & Ridge Regression.
...
<mark>Based on previous result of the `Auto` dataset, I intuitively choose variables `displacement` and `weight` to train and build an SVM classifier to separate high and low mpg cars.
...
<mark>Based on the error of the cross-validation (and the summaries), we conclude that `cost = 0.1` leads to the lowest error rate. Cost value lower than $0.1$ leads to higher error rates, suggesting evidence of underfitting; while higher cost values indicating overfitting since the model is too complex to account for the testing data.
...
<mark>Based on the tune results, we can see that `cost` equals to $10$ achieved the smallest error rates.

### (e)

<mark>We use `cost = 10` to compute the training and test error rates.
...
<mark>We can see that after applying the optimal cost $10$, training error rate remained the same, while testing error slightly increased from $0.1222222$ to $0.1259$. This might suggest `cost = 10` causes the model to slightly overfit on the training data.
