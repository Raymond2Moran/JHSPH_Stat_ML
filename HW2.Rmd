...
Therefore, we confirm that this $\alpha$ minimizes $\mathrm{Var}\Big(\alpha X + (1-\alpha)Y\Big)$.

## Question 2 (5.4-3)

### (a)

For the entire dataset, $k$-fold cross validation is implemented by dividing it into $k$ parts of equal size. The model will be fitted $k$ times, with one of the $k$ subsets serving as the test set and the remaining $k-1$ subsets forming training set. The average error across $k$ trials is computed at the end.

### (b)

#### (i)

Advantage of $k$-fold CV compared to validation set:
...
Disadvantage of $k$-fold CV compared to validation set:
...
Advantage of $k$-fold CV compared to LOOCV:
...
Disadvantage of $k$-fold CV compared to LOOCV:
...
From the output test errors, we may safely assume that our method may not have much variance.
...
From the result, we can see that `student` as a dummy variable does not changes the test error too much.
...
We conclude from the table that the standard errors correspond to `income` and `balance` equal to $4.985e-06$ and $2.274e-04$ respectively.
...
The standard errors from the bootstrapping and from the previous `glm()` call are very similar.

## Question 5 (6.8-1)

### (a)

Best subset model with $k$ predictors has the smallest training error.

### (b)

We don't know any information regarding the test set, so it could be any of these three model.

### (c)

#### i.

True. This is the definition of forward stepwise selection.

#### ii.

True. This is the definition of backward stepwise selection.

#### iii.

False. The predictors of backward / forward stepwise selection at a particular step depends on the previous stage of the selection process. It is not dependent on the other selection strategy.

#### iv.

False. For the same reason of iii.

#### v.

False. The selection of variables using the best subset model **solely** depends on the subset that minimizes RSS on the training set. So it is not necessarily nested for $k$ and $k+1$ predictors.
...
From the result, we can see that adjusted $R^2$, $BIC$, and $C_p$ all choose the 4-variable model. Therefore, the best model is the 4 variables listed above.
...
From the summary result, we can see that Lasso model chooses the best model, which is the same as the previous forward stepwise selection $BIC$ model.
...
The $BIC$ based on the best subset selection model AND the Lasso model identified the correct model. The best subset selection based on adjusted $R^2$ included too many variables.
...
Therefore, the RMSE on the test set is $1079.981$.
...
Thus, the RMSE on the test set for Ridge regression is $1137.579$.
...
The RMSE on the test set of Lasso regression is $1087.336$. There are 16 non-zero coefficient estimates (including the intercept).
...
pcr_rmse
```
The value of $M$ selected by CV is $17$;
the RMSE on the test set for PCR fit is $1079.981$. 
...
pls_rmse
```
The value of $M$ selected by CV is $16$; the RMSE on the test set is $1079.969$.
...
It seems that Ridge regression performed a little bit off compared to the rest of the models. PCR & PLS model seem to perform equally well compared to the simple linear model.