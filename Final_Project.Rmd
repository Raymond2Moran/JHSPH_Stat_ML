...
cat("\nDimension of the data frame after dropping NA's in `mortstats`:\n")
print(dim(data))

# Re-check percentage of NA values

percent_na <- sapply(data, function(col) sum(is.na(col)) / length(col) * 100)
cat("\nPercentage of NA values after filtering for mortality statuses:\n")
print(percent_na)
...
cat("\nPercentage of NA values after imputation:\n")
print(percent_na)
```
...
confusion_matrix_logistic_best <- confusionMatrix(factor(test_pred_binary), factor(test_set$mortstat), positive = "1") # Set the positive class to 1 so that 
...
cat("\nRoot Mean Squared Error (MSE) of Linear Discriminant Analysis (LDA):", rmse_lda, "\n\n")
...
cat("\nRoot Mean Squared Error (RMSE) Quadratic Discriminant Analysis (QDA):", rmse_qda, "\n\n")
...
The result, however, suggested that the model is underfiting the data compared with training the model using the full set of variables. Both accuracy & sensitivity decreased compared to the previous decision tree model.

#### Random Forest (with and w/o Bagging)

For random forest model, we proceeded with similar procedures compared to the previous decision tree model. First, we run the random forest model using the full set of variables (continuous scaled). Then, based on importance plot,
...
confusion_matrix <- confusionMatrix(test_pred_binary, actual_values, positive = "1")

print("Confusion Matrix for Random Forest:")
print(confusion_matrix)

rmse_rf <- sqrt(mean((as.numeric(test_pred) - as.numeric(test_set$mortstat))^2))
cat("Root Mean Squared Error (MSE) of Random Forest:", rmse_rf, "\n")
...
rf_bagging <- randomForest(mortstat ~ RIDAGEYR + BPXSY1 + BPXDI1 + LBXRDW + BMXBMI + BMXWAIST, data = train_set, mtry = 6 - 1, ntree = 500, importance = TRUE) # Perform bagging, all features were considered when trying to split the node
...
best_shrinkage <- shrinkage_values[which.min(sapply(results, function(x) x$cv_error))]

cat("Best shrinkage value:", best_shrinkage, "\n")

# Retrieve the best model based on shrinkage
...
confusion_matrix <- confusionMatrix(as.factor(test_preds_binary), as.factor(test_set$mortstat), positive = "1")
...
alive_group_raw <- imputed_data %>% filter(mortstat == 0) %>% dplyr::select(-SEQN)
deceased_group_raw <- imputed_data %>% filter(mortstat == 1) %>% dplyr::select(-SEQN)

set.seed(123456)

alive_train_ind_raw <- sample(seq_len(nrow(alive_group_raw)), size = floor(0.8 * nrow(alive_group_raw)))
alive_train_raw <- alive_group_raw[alive_train_ind_raw, ]
alive_test_raw <- alive_group_raw[-alive_train_ind_raw, ]

deceased_train_ind_raw <- sample(seq_len(nrow(deceased_group_raw)), size = floor(0.8 * nrow(deceased_group_raw)))
deceased_train_raw <- deceased_group_raw[deceased_train_ind_raw, ]
deceased_test_raw <- deceased_group_raw[-deceased_train_ind_raw, ]

train_set <- bind_rows(alive_train_raw, deceased_train_raw)
test_set <- bind_rows(alive_test_raw, deceased_test_raw)

train_set$mortstat <- as.factor(train_set$mortstat)
test_set$mortstat <- as.factor(test_set$mortstat)


x_train <- train_set %>% dplyr::select(-mortstat)
y_train <- train_set$mortstat

x_test <- test_set %>% dplyr::select(-mortstat)
y_test <- test_set$mortstat 


svm_linear_tune <- tune(svm, mortstat ~ RIDAGEYR + BPXSY1 + BPXDI1 + LBXRDW, data = train_set, kernel = "linear", ranges = list(cost = c(0.001, 0.01, 0.1, 1, 10)))
...
test_preds_binary <- ifelse(test_pred_linear >= 0.5, 1, 0)
confusion_matrix_linear <- confusionMatrix(as.factor(test_preds_binary), as.factor(test_set$mortstat), positive = "1")
confusion_matrix_linear

test_pred_numeric <- as.numeric(test_pred_linear) - 1
actual_values_numeric <- as.numeric(test_set$mortstat) - 1


mse_linear <- mean((test_pred_numeric - actual_values_numeric)^2)
...
svm_tune_rad <- tune(svm, 
                     mortstat ~ ,                   # Formula with all predictors
                     data = train_set,               # Your training dataset
                     kernel = "radial",              # Radial kernel
                     ranges = list(cost = cost, gamma = gamma))  # Grid for hyperparameters
...
confusion_matrix_radial <- confusionMatrix(test_pred_radial, test_set$mortstat)
confusion_matrix_radial

degrees <- c(2, 3, 4)

svm_tune_poly <- tune(svm, mortstat ~ ., data = train_set, kernel = "polynomial", ranges = list(cost = cost, degree = degrees))

summary(svm_tune_poly)

best_svm_model <- svm_tune_poly$best.model
test_pred_poly <- predict(best_svm_model, newdata = test_set)
confusion_matrix_poly <- confusionMatrix(test_pred_poly, test_set$mortstat)
